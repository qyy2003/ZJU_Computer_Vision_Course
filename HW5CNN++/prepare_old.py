...
training_data= PokemonImageDataset(train_dataset,img_dir='images/',label_dict=dict_all,is_transform=True,repeat=10)
...
if __name__ == "__main__":
    ...
    acc_list=[]
    def to_binary(y):
        new_label=torch.zeros([y.size(0)])
        return torch.eye(len(labels))[y]
    for epoch in range(epochs):
        ...
        for batch_idx, (x, y) in tqdm(enumerate(train_data_loader, 1)):
            ...
            for y1,y2 in zip(pred_y,y):
                total_acc1 +=y2[y1[0]] if y1[1]==NONE else y2[y1[0]] *0.5
                total_acc1 +=y2[y1[1]] if y1[0]==NONE else y2[y1[1]] *0.5
        ...
        for batch_idx, (x, y) in tqdm(enumerate(valid_data_loader, 1)):
            ...
            pred_y = torch.sort(pred_y, dim=1,descending=True)[1]
            for y1,y2 in zip(pred_y,y):
                total_acc +=y2[y1[0]] if y1[1]==NONE else y2[y1[0]] *0.5
                total_acc +=y2[y1[1]] if y1[0]==NONE else y2[y1[1]] *0.5
